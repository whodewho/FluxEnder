...
def insert_nor_sus(_coll, key, _dict ):
    _coll.insert({"_id":key, "ITEMS":_dict["ITEMS"], "TTLS":_dict["TTLS"], \
...
def update_nor_sus(_coll, key, _dict, _doc):
    if _doc == None:
        ...
        if len(new_ttl_list) > 0:
            _coll.update({"_id":key},{"$push":{"TTLS":{"$each":new_ttl_list}}})
        ...
        if item in index_dict:
            ...
            for ttl in _dict[item]["TTLS"]:
                if ttl not in _doc[index]["TTLS"]:
                    new_ttl_list.append(ttl)
            if len(new_ttl_list) > 0:
                _coll.update({"_id":key}, {"$push":{"ITEMS."+str(index)+".TTLS":{"$each":new_ttl_list}}})
            if _dict[item]["FIRST_SEEN"] < _doc[index]["FIRST_SEEN"]:
                _coll.update({"_id":key}, {"$set":{"ITEMS."+str(index)+".FIRST_SEEN":_dict[item]["FIRST_SEEN"]}})
            if _dict[item]["LAST_SEEN"] > _doc[index]["LAST_SEEN"]:
                _coll.update({"_id":key}, {"$set":{"ITEMS."+str(index)+".LAST_SEEN":_dict[item]["LAST_SEEN"]}})
            _coll.update({"_id":key}, {"$set":{"ITEMS."+str(index)+".COUNT":_dict[item]["COUNT"]+_doc[index]["COUNT"]}})
        else:
            new_dict = generate_one_dict(_dict[item], item) 
            _coll.update({"_id":key}, {"$push":{"ITEMS":new_dict}})

    if (len(index_dict) + len(_dict) - number) > 100 :
...
def dump_dict(_coll, nor_coll, sus_coll, nor_set, sus_set, _dict, domain_flag): 
    for key1 in _dict:
        if key1 in sus_set:
            ...
            if _doc == None:
                ...
                update(_coll, _dict[key1], item_list, key1, sus_coll, sus_set) 
                
def init_set(coll, sus_set):
    cursor = coll.find({},{"_id":1})
    for item in cursor:
        sus_set.add(item["_id"])

def init_nor_domain_set(f, nor_set):
    for line in open(f):
...
for f in file_list:
    ...
    bar2 = time.clock()
    dump_dict(db.domain, db.nor_domain, db.sus_domain, nor_domain_set, sus_domain_set, domain_dict, True)
    ...
    bar3 = time.clock()
    dump_dict(db.ip, db.nor_ip, db.sus_ip, nor_ip_set, sus_ip_set, ip_dict, False) 
